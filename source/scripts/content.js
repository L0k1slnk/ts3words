...
chrome.runtime.sendMessage({message: "get data"}, function (response) {
    ...
    if (response) {
        ...
        if (response.ts3Words.active) {
            ...
            removeFindedWords();
            searchWord(ts3Words.words[0].word);
        }
...
function searchWord(word) {
    var options = {
        ...
        each: function (el) {
            console.log($(el));
            addWordsMarkup($(el), ts3Words.words[0]);
            checkVisibility($(el));
            var bt = $('.__ts3w-word__lng');
            var turbVal = {val: 0.000001};
            var turb = $('#__ts3w-word-filter feTurbulence');
            var btTl = new TimelineLite({
                paused: true, onUpdate: function () {
                    console.log('baseFrequency', turbVal.val);
                    turb[0].setAttribute('baseFrequency', turbVal.val);
                }
            });

            btTl.to(turbVal, 0.3, {val: 0.02});
            btTl.to(turbVal, 0.3, {val: 0.000001});

            bt.off('click.ts3wSettings').on('click.ts3wSettings', function () {
                console.log(bt);
                btTl.restart();
            });
        },
...
function addWordsMarkup($el, word) {
    var markup = '';
    markup += '<ts3w class="__ts3w-word__lng __ts3w-word__lng--current">' + word.word + '</ts3w>';
